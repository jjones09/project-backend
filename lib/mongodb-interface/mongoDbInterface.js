...
const connConf = require('../../config/mongodbConfig.json');
    ...
    return new Promise((resolve, reject) => {
        if (!connection) {
            mongodb.connect(buildConnectionString(), (err, db) => {
                if (err) {
                    console.log('Error connection to mongodb-interface: %s', err.message);
                    reject(err.message);
                }
                else {
                    console.log('Connected to mongodb-interface');
                    connection = db;
                    resolve(connection);
                }
            });
