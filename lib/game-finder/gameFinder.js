...
let getResults = (type, query) => {
    return db.findGames(type, query);
};

let addGamesToDatabase = (games) => {
    return db.addGames(games);
};
...
module.exports = {
    ...
    findGames: (type, query) => {
        return new Promise((resolve) => {
           getResults(type, query).then(games => {
